...
private:
	...
	void search_node(node_ptr node, const KeyType& key, bool& found, node_ptr* out_node);
...
template <typename KeyType, typename ValueType>
void Treap<KeyType, ValueType>::search_node(node_ptr node, const KeyType& key, bool& found, node_ptr* out_node)
{
	if (node == nullptr)
	{
		found = false;
	...
	if (node->key < key)
	{
		/*return */this->search_node(node->childr, key, found, out_node);
	}
	else
	{
		/*return */this->search_node(node->childl, key, found, out_node);
	}
}
